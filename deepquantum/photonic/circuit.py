...
class QumodeCircuit(Operation):
    ...
    def loss(
        ...
        if sigma is None:
            sigma = self.sigma
        ps = Loss(inputs=0, nmode=self.nmode, wires=wires, cutoff=self.cutoff,
                        requires_grad=requires_grad, noise=self.noise, mu=mu, sigma=sigma)
        self.add(ps, encode=encode)

        # para = np.log(10)/10
        # print(self.init_state.cov)
        # self.init_state.cov = self.init_state.cov * np.exp(-para*loss) \
        #                     + (1-np.exp(-para*loss)) * torch.eye(2*self.nmode)
        # print(self.init_state.cov)
